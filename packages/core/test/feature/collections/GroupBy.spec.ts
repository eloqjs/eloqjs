...
describe('Feature – Collections – GroupBy', () => {
  ...
  it('should support a closure', () => {
    ...
    const groupedCollections = collection.groupBy((model) => String(model.price).substring(0, 2))
  ...
  it('should use an empty string as the key ' + 'if objects are missing the key to group by', () => {
    ...
    assertCollection(groupedCollections[''], [{ id: 4, name: 'Table', price: null, manufacturer: null }])
  })
})
